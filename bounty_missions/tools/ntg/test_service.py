...
""".strip(),
    ...
    def test_sync_external_pr_history_records_invalid_feedback_on_closed_pr(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            ledger_path = root / "bounty_ledger.json"
            mission_log = root / "mission_log.md"
...
""".strip(),
            ...
            with mock.patch(
                ...
                return_value={
                    16285: {
                        ...
                        "title": "Add template for CVE-2020-10987 (Tenda AC15 RCE)",
                        "html_url": "https://github.com/projectdiscovery/nuclei-templates/pull/16285",
                        "latest_comment_author": "DhiyaneshGeek",
                        "latest_comment_created_at": "2026-05-28T16:51:10Z",
                        "latest_comment_body": "We observed that these changes are not valid and appear to have been generated by Gemini AI. If you believe these templates are valid, kindly share the setup instructions or a vulnerable setup from your end.",
                    }
                },
            ):
                payload = sync_external_pr_history(
            ...
            self.assertEqual(item["closure_reason"], "invalid_generated_template")
            self.assertEqual(item["review_feedback_author"], "DhiyaneshGeek")
            self.assertIn("setup instructions", item["review_feedback_excerpt"])
